...
use vk_assist::structures::{get_rect_as_basic, SimpleVertex};
...
impl VulkanApp {
    pub fn new(event_loop: &winit::event_loop::EventLoop<()>) -> VulkanApp {
        ...
        //init device
        let vulkan_device = VulkanDevice::create_device(
            &instance,
            &vulkan_surface,
        ...
        let sync_ojbects =
            share::v1::create_sync_objects(&vulkan_device.logical_device, MAX_FRAMES_IN_FLIGHT);
...
impl VulkanApp {
    fn create_graphics_pipeline(
        ...
        let vert_shader_module = share::create_shader_module(
            device,
            include_bytes!("../shaders/vertexbuffer.vert.spv").to_vec(),
        );
        let frag_shader_module = share::create_shader_module(
            device,
            include_bytes!("../shaders/vertexbuffer.frag.spv").to_vec(),
        );
        ...
        let binding_description = SimpleVertex::get_binding_description();
        let attribute_description = SimpleVertex::get_attribute_descriptions();
    ...
    fn draw_frame(&mut self) {
        ...
        let result = unsafe {
            self.vulkan_swap_chain
                .swapchain_loader
                .queue_present(self.vulkan_device.present_queue, &present_info)
        };
...
impl Drop for VulkanApp {
    fn drop(&mut self) {
        unsafe {
            ...
            self.vulkan_device.logical_device.destroy_device(None);
            self.vulkan_surface
                .surface_loader
